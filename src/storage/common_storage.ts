...
/**
 * Process the module content to update it with a new name during rename/copy operations.
 * This regenerates the Python code and exported blocks to ensure they use the new name.
 */
export function processModuleContentForRename(
  oldProjectName: string, oldModuleName: string, newProjectName: string, newModuleName: string,
  moduleContent: string): string {
  const parts = getParts(moduleContent);
  let blocksContent = parts[PARTS_INDEX_BLOCKS_CONTENT];
  if (blocksContent.startsWith(MARKER_BLOCKS_CONTENT)) {
    blocksContent = blocksContent.substring(MARKER_BLOCKS_CONTENT.length);
  ...
  // Create the updated module info
  const module: Module = {
    modulePath: makeModulePath(newProjectName, newModuleName),
    moduleType: moduleType,
    projectName: newProjectName,
    moduleName: newModuleName,
    dateModifiedMillis: Date.now(),
    className: moduleNameToClassName(newModuleName),
  };

  // Update blocks content that reference the old names
  const updatedBlocksContent = updateBlocksContentForRename(
    oldProjectName, oldModuleName, newProjectName, newModuleName, blocksContent);

  // Create a headless blockly workspace to regenerate Python code
  const headlessBlocklyWorkspace = new Blockly.Workspace();
  headlessBlocklyWorkspace.options.oneBasedIndex = false;
  Blockly.serialization.workspaces.load(
    JSON.parse(updatedBlocksContent), headlessBlocklyWorkspace);

  const generatorContext = createGeneratorContext();
  generatorContext.setModule(module);

  // Regenerate Python code with the new names
  const pythonCode = extendedPythonGenerator.mrcWorkspaceToCode(
    headlessBlocklyWorkspace, generatorContext);
  const exportedBlocks = JSON.stringify(generatorContext.getExportedBlocks());
  const components = '[]';
  ...
  return makeModuleContent(
    module, pythonCode, updatedBlocksContent, exportedBlocks, components);
}

/**
 * Updates blocks content to replace references to old names with new names.
 * This includes updating blocks that reference module names, class names, etc.
 */
function updateBlocksContentForRename(
  oldProjectName: string, oldModuleName: string, newProjectName: string, newModuleName: string,
  blocksContent: string): string {
  try {
    const blocksData = JSON.parse(blocksContent);

    // Recursively update all blocks in the workspace
    updateBlocksRecursive(blocksData, oldProjectName, oldModuleName, newProjectName, newModuleName);

    return JSON.stringify(blocksData);
  } catch (e) {
    console.log('Failed to parse or update blocks content:', e);
    return blocksContent; // Return original if parsing fails
  }
}


//Recursively updates blocks in the given object, replacing old project/module names with new ones.
function updateBlocksRecursive(obj: any, oldProjectName: string, oldModuleName: string,
  newProjectName: string, newModuleName: string): void {
  ...
  if (Array.isArray(obj)) {
    obj.forEach(item => updateBlocksRecursive(item, oldProjectName, oldModuleName, newProjectName, newModuleName));
    return;
  }
  ...
  for (const key in obj) {
    if (obj.hasOwnProperty(key)) {
      ...
      // Update string values that match old names
      if (typeof value === 'string') {
        // Update project name references
        if (value === oldProjectName) {
          obj[key] = newProjectName;
        }
        // Update module name references
        else if (value === oldModuleName) {
          obj[key] = newModuleName;
        }
        // Update class name references (PascalCase versions)
        else if (value === moduleNameToClassName(oldModuleName)) {
          obj[key] = moduleNameToClassName(newModuleName);
        }
        // Update module paths
        else if (value === makeModulePath(oldProjectName, oldModuleName)) {
          obj[key] = makeModulePath(newProjectName, newModuleName);
        }
        // Update project paths
        else if (value === makeProjectPath(oldProjectName)) {
          obj[key] = makeProjectPath(newProjectName);
        }
      }
      // Recursively process nested objects and arrays
      else if (typeof value === 'object') {
        updateBlocksRecursive(value, oldProjectName, oldModuleName, newProjectName, newModuleName);
      }
    }
  }
}
...
//Processes module content for project renaming, updating all modules in the project 
export function processModuleContentForProjectRename(
  oldProjectName: string, newProjectName: string, moduleContent: string): string {
  const parts = getParts(moduleContent);
  let moduleType = parts[PARTS_INDEX_MODULE_TYPE];
  if (moduleType.startsWith(MARKER_MODULE_TYPE)) {
    moduleType = moduleType.substring(MARKER_MODULE_TYPE.length);
  ...
  // For project rename, the module name depends on the module type
  let oldModuleName: string;
  let newModuleName: string;

  if (moduleType === MODULE_TYPE_PROJECT) {
    oldModuleName = oldProjectName;
    newModuleName = newProjectName;
  } else {
    const originalModulePath = extractOriginalModulePath(moduleContent);
    if (originalModulePath) {
      oldModuleName = getModuleName(originalModulePath);
      newModuleName = oldModuleName; // Keep the same module name, just change project
  ...
  return processModuleContentForRename(
    oldProjectName, oldModuleName, newProjectName, newModuleName, moduleContent);
}

//Helper function to extract the original module path from the module content
function extractOriginalModulePath(moduleContent: string): string | null {
  const parts = getParts(moduleContent);
  let blocksContent = parts[PARTS_INDEX_BLOCKS_CONTENT];
  if (blocksContent.startsWith(MARKER_BLOCKS_CONTENT)) {
    blocksContent = blocksContent.substring(MARKER_BLOCKS_CONTENT.length);
  ...
  try {
    ...
    return findModulePathInBlocks(blocksData);
  } catch (e) {
    return null;
  }
}

//Recursively searches through the blocks data to find a module path 
function findModulePathInBlocks(obj: any): string | null {
  if (!obj || typeof obj !== 'object') {
    return null;
  }
  ...
  if (Array.isArray(obj)) {
    for (const item of obj) {
      const result = findModulePathInBlocks(item);
  ...
  for (const key in obj) {
    if (obj.hasOwnProperty(key)) {
      ...
      // Look for paths that match the pattern projectName/moduleName.py
      if (typeof value === 'string' && value.includes('/') && value.endsWith('.py')) {
        const regex = /^([a-z_][a-z0-9_]*)\/([a-z_][a-z0-9_]*).py$/;
        if (regex.test(value)) {
          return value;
        }
      }

      const result = findModulePathInBlocks(value);
      if (result) return result;
    }
  }
